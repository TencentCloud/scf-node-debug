...
// internal depend
const { scfConfig, suffixBinMap } = require('../config')
const logger = require('../lib/logger')
const utils = require('../helper/utils')
const jsCaller = require('../lib/jsCaller')
const phpCaller = require('../lib/phpCaller')
const pythonCaller = require('../lib/pythonCaller')
...
module.exports = function(entry, handler, timeout) {
  return async function(ctx, next) {
    ...
    ***************************************************/
    ...
    let childTaskEnv = Object.assign(process.env, {
      entry: path.resolve(process.cwd(), entry),
      handler,
      event: JSON.stringify(ctx.testModel)
    })

    if (fs.existsSync(utils.tmpFile)) {
      fs.unlink(utils.tmpFile)
    }
    if (fs.existsSync(utils.responseFile)) {
      fs.unlink(utils.responseFile)
    }

    try {
      switch (extname) {
        case '.js':
          childProcess = jsCaller(childTaskEnv)
          break
        case '.py':
          childProcess = pythonCaller(childTaskEnv)
          break
        case '.php':
          childProcess = phpCaller(childTaskEnv)
          break
        default:
          throw '当前文件后缀不支持，目前仅支持本地测试js,python,php程序'
      }
    } catch (e) {
      logger.error(e)
    }
    ...
    ***************************************************/
    ...
    childProcess.on('exit', code => {
      const resFile = fs.readFileSync(utils.responseFile, {
        flag: 'a+'
      })
      if (resFile) returnVal = JSON.parse(JSON.stringify(resFile.toString()))
      isDone = true
      exitCode = code
    })
